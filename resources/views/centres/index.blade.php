                    ...
                    <div class="flex justify-end gap-3 px-1 lg:px-4">
                        <div class="mb-4 pb-5">
                            <div class="relative">
                                ...
                                <form>
                                    <input type="text" id="search" name="search" value="{{ request('search') }}" class="block p-1.5 pl-10 w-80 text-sm text-color_rem bg-gray-50 rounded-lg border border-gray-300 focus:ring-color_att focus:border-ventis_color" placeholder="Rechercher par nom/province/commune">
                                </form>
                            </div>
                        </div>
                        <span class="mb-4 pb-5 p-1">
                            <a href="{{ route('centre.create') }}" title="Créer ici..">
                                ...
                                @forelse ($datas as $data)
                                    <tr class="hover:bg-color_rej">
                                        ...
                                        <td class="px-4 py-2 text-sm font-normal whitespace-nowrap">
                                            <div>
                                                <p class="text-sm text-gray-600">
                                                    {{ $data?->laProvince?->code."/".$data?->laCommune?->libelle."/".$data?->adresse }}
                                                </p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-2 text-sm font-normal whitespace-nowrap">
                                            <div>
                                                <p class="text-sm text-gray-600">
                                                    @if($data?->le_tat?->libelle == "En circuit")
                                        ...
                                        <td class="px-3 py-2 text-sm whitespace-nowrap flex items-center justify-start">
                                            <a href="{{route('centre.edit', $data?->id)}}" class="font-light hover:underline" title="Modifier">
                                            ...
                                            <a href="{{ route('centre.destroy', $data?->id) }}" title="Supprimer" class="font-light hover:underliner">
                                                <form method="post" action="{{route('centre.destroy', $data?->id)}}" enctype="multipart/form-data">
                                                    @csrf
                                                    @method("DELETE")
