                    ...
                    <div class="flex justify-end gap-3 px-1 lg:px-4">
                        <div class="mb-4 pb-5">
                            <div class="relative flex">
                                <form action="{{route('examen.index')}}" method="get" class="flex gap-1">
                                    <select name="selected_session" class="p-1.5 pl-3 w-56 text-sm text-color_rem bg-gray-50 rounded-lg border border-gray-300 focus:ring-color_att focus:border-ventis_color">
                                        <option disabled selected>Sélectionner ici..</option>
                                        @foreach ($les_sessions as $session)
                                            <option value=" {{ $session->session_exam }} ">Session {{ $session->session_exam }}</option>
                                        @endforeach
                                    </select>
                                    <button type="submit" class="flex items-center px-2 py-1.5 text-sm text-color_att hover:text-white tracking-wide transition-colors duration-200 bg-mycolor3 rounded-lg shrink-0 sm:w-auto gap-x-1 hover:bg-ventis_color">
                                        <svg class="w-4 h-4" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path></svg>
                                    </button>
                                </form>
                            </div>
                        </div>
                        <a href="{{ route('examen.create') }}" title="Créer ici..">
                            <button class="flex items-center px-2 py-1.5 text-sm tracking-wide text-color_rej transition-colors duration-200 bg-color_att rounded-lg shrink-0 sm:w-auto gap-x-1 hover:bg-ventis_color">
                                ...
                                @forelse ($datas as $data)
                                    <tr class="hover:bg-color_rej">
                                        ...
                                        <td class="px-4 py-2 text-sm font-normal whitespace-nowrap">
                                            <div>
                                                <p class="text-sm text-gray-600">
                                                    @if($data?->leStatut?->libelle == "Activé")
                                                        <span class="shadow-md p-1 hover:shadow-lg hover:shadow-color_rej text-sfe_color">
                                                            {{ $data?->leStatut?->libelle }}
                                                        </span>
                                                    @elseif($data?->leStatut?->libelle == "Encours d'exécution")
                                                        <span class="fade-in shadow-md p-1 hover:shadow-lg hover:shadow-color_rej text-color_att">
                                                            {{ $data?->leStatut?->libelle }}
                                                        </span>
                                                    @else
                                                        <span class="shadow-md p-1 hover:shadow-lg hover:shadow-color_rej text-red-800">
                                                            {{ $data?->leStatut?->libelle }}
                                                        </span>
                                                    @endif
                                                </p>
                                        ...
                                        <td class="px-3 py-2 text-sm whitespace-nowrap flex items-center justify-start">
                                            <a href="{{route('examen.edit', $data?->id)}}" class="font-light hover:underline" title="Modifier">
                                            ...
                                            <a href="{{ route('examen.destroy', $data?->id) }}" title="Supprimer" class="font-light hover:underliner">
                                                <form method="post" action="{{route('examen.destroy', $data?->id)}}" enctype="multipart/form-data">
                                                    @csrf
                                                    @method("DELETE")
