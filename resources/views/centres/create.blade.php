                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4">
                        <fieldset>
                            <div class="pt-4">
                                ...
                                <div class="relative mt-1">
                                    <input type="text" name="nom_centre" id="nom_centre" value="{{ old('nom_centre') }}" required maxlength="155" class="peer block w-full border-0 bg-gray-50 py-0.5 text-gray-900 focus:ring-0 sm:text-sm sm:leading-6">
                                    @error('nom_centre')
                            ...
                            <div class="pt-4">
                                ...
                                <div class="relative mt-1">
                                    <select name="province_id" class="form-select peer block w-full border-0 bg-gray-50 py-0.5 text-gray-900 focus:ring-0 sm:text-sm sm:leading-6">
                                        ...
                                        @forelse  ($les_provinces as $v)
                                            <option id="provinces" value=" {{ $v->id }} "> {{ $v?->libelle }}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                    @error('province_id')
                        ...
                        <fieldset>
                            <div class="pt-4">
                                ...
                                <div class="relative mt-1">
                                    <input type="text" name="adresse" id="adresse" value="{{ old('adresse') }}" required class="peer block w-full border-0 bg-gray-50 py-0.5 text-gray-900 focus:ring-0 sm:text-sm sm:leading-6">
                                    @error('adresse')
                                    <div class="text-red-500 text-sm">{{$message}}</div>
                                    @enderror
                                    <div class="absolute inset-x-0 bottom-0 border-t border-ventis_color peer-focus:border-t-2 peer-focus:border-color_att" aria-hidden="true"></div>
